...
export default function DiscoverSaveToGalleryDialog({ open, onOpenChange, post, galleries, onSaveToExisting, onSaveToAllPhotos, onCreateNew, isSaving }) {
      ...
      <DialogContent className="bg-card border-border max-w-sm max-h-[85vh] overflow-y-auto">
        ...
        {mode === 'pick' && (
          <div className="space-y-2">
            {/* All Photos option */}
            <button
              onClick={onSaveToAllPhotos}
              disabled={isSaving}
              className="w-full flex items-center gap-3 px-4 py-3 rounded-2xl border border-primary/30 bg-primary/5 hover:bg-primary/10 transition-all text-left"
            >
              <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center shrink-0">
                <Images className="w-4 h-4 text-primary" />
              </div>
              <div className="flex-1">
                <p className="font-dm text-sm font-medium text-primary">All Photos</p>
                <p className="font-dm text-[11px] text-muted-foreground">Add without picking a gallery yet</p>
              </div>
            ...
            {/* Existing gallery-type templates only */}
            {galleries.length > 0 && (
              <>
